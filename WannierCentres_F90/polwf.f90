MODULE runparam
 ...
 IMPLICIT NONE
 INTEGER :: tbeg, nt, nsp, nst, nskp, corrs, ncmax, crdstyle, inpunits
 LOGICAL :: rmode, wfadj
 ...
 DO ia = 1,na(is)
  ...
  IF (sw == 0) THEN
   ...
   DO ist = 1,nst
    ...
    IF (rsep <= rloc) THEN
     nep(is,ia) = nep(is,ia) + 2
     wftg(ist) = 1
     IF (rsep > 2.0d0) WRITE(6,*) ist, 'IS ', rsep,'AWAY FROM (1)', is, ia
    END IF
!    WRITE(6,*) "WF ",ist," :",rsep
   END DO
